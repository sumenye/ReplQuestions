...
public class Book {
	 ...
	 */
	    ...
	    public static String capitalize(String word) {
	    	...
	    	for(String each: ch) {
	    		newstr= each.substring(0, 1).toUpperCase()+ each.substring(1).toLowerCase ();
	    	}
	 ...
	 //   }
	    ...
	    public static Boolean isTitleWord(String word) {
			String [] str= {"a" , "an", "the","and", 
					"but", "for", "nor", "or", "so", "yet", "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"} ;
	    	boolean b= false;
	    ...
	    public void setTitle(String title) {
	    	...
	    	this.title= capitalize(splitted[0]);
	    	for(int i=1;i<splitted.length;i++){
	              if(!isTitleWord(title))
	              this.title=capitalize(splitted[i]);
	              else if(isTitleWord(title))
	              splitted[i].toLowerCase().trim();
	            }
	  ...
	  return "Book[author=" + this.getAuthor() + ", title= " + this.getTitle() + ", id=" +this.getId() + ", pages=" + this.getPages()+"]";
